...
const SPECIALS=[
 ...
 {id:'para',  n:'日傘',          t:'m',abs:1,d:16,cd:420,rc:52,e:{guard:22,sun:1,block:1},desc:'サブ動作で受け（被ダメ65%減）。Xキーで太陽光を反射し、前方の相手をスタンさせる（4秒に1回・攻撃と同時に出せる）。'},
...
function meleeHit(e,d,dm,w){
 ...
 // CPUは攻撃に合わせて自動で光らせる（プレイヤーはXキーで好きな時に）
 if(d.e.sun&&e.bot)sunFlash(e);
 // 斬撃エフェクト
...
/* 日傘：太陽光の反射で前方の相手をスタンさせる。
   攻撃のクールダウンを消費しないので、光らせながら斬れる。 */
function sunFlash(e){
 if(e.sun>0){
  if(e===player)addText(e.x,e.y-34,'あと'+(e.sun/1000).toFixed(1)+'秒','#8a93a5');
  return false;
 }
 e.sun=4000;
 let n=0;
 for(const t of ents){
  if(t===e||!t.alive)continue;
  if(Math.hypot(t.x-e.x,t.y-e.y)>320)continue;
  if(Math.abs(angDiff(Math.atan2(t.y-e.y,t.x-e.x),e.ang))>0.7)continue;
  t.stun=Math.max(t.stun,1100);
  addText(t.x,t.y-24,'まぶしい!','#ffe9a0');
  n++;
 }
 addFx(e.x+Math.cos(e.ang)*50,e.y+Math.sin(e.ang)*50,'#fff3b0',22,5,500,4);
 fx.push({slash:1,x:e.x,y:e.y,a:e.ang,rc:330,l:260,ml:260,c:'#fff3b0',arc:0.7});
 if(e===player)addText(e.x,e.y-34,n?'まぶしい光！ '+n+'人':'まぶしい光！','#ffe9a0');
 return true;
}
...
function uiButtons(){
 ...
 const xLabel=d&&d.e.sun?'眩':(d&&d.e.barrier?'纏い':(d&&d.e.elem?['火','雷','水','土'][player.elem]:'—'));
 const list=[
  ...
  {id:'x',   x:c1,y:b0-gap*3, r:R,l:xLabel,s:d&&d.e.sun?(player.sun>0?(player.sun/1000).toFixed(0)+'秒':'光る'):'特殊',on:!!(d&&(d.e.barrier||d.e.elem||d.e.sun))&&!(d&&d.e.sun&&player.sun>0)},
...
function drawHUD(t){
 ...
 if(cw&&wdef(cw).e.sun){
  ...
  ctx.fillStyle=P.sun>0?'#7f93aa':'#ffe9a0';
  ctx.fillText(P.sun>0?'眩しい光　あと '+(P.sun/1000).toFixed(1)+'秒':'Xキーで眩しい光（前方をスタン・攻撃と同時に出せる）',W/2,H-158);
 } else if(cw&&wdef(cw).e.barrier){
  ctx.textAlign='center';ctx.font='12px system-ui';
  let msg,col;
